...
Core tests take longer than any other Silicon tests, due to the high amount of computational work involved in validating core components.
...
To run only Silicon's core tests (after building):
...
To run only Silicon's crypto tests (after building):
...
To run only Silicon's hash tests (after building):
...
To run only Silicon's performance tests (after building):
...
To run only Silicon's unit tests (after building):
